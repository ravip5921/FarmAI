...
class LineDetectionStage(PipelineStage):
    ...
    def _directional_kernel_sizes(
        self, binary: np.ndarray, char_height: int
    ) -> tuple[int, int, int]:
    ...
    def _close_gaps(
        self, binary: np.ndarray, *, orientation: str, gap: int
    ) -> np.ndarray:
        if gap <= 1:
            return binary
    ...
    def _hough_vertical_mask(
        self, binary: np.ndarray, min_line_length: int
    ) -> np.ndarray:
    ...
    def process(self, doc: DocumentImage) -> DocumentImage:
        ...
        report = estimate_character_size(binary, min_area=10)
        char_height = max(1, report.average_height or report.median_height or 10)
        size, vertical_size, page_kernel_size = self._directional_kernel_sizes(
            binary, char_height
        )
        horizontal_gap = max(1, min(max(1, size // 4), max(1, binary.shape[1] // 180)))
        vertical_gap = max(
            1, min(max(1, vertical_size // 2), max(1, binary.shape[0] // 180))
        )
        ...
        horizontal_mask = cv2.morphologyEx(
            horizontal_source, cv2.MORPH_OPEN, horizontal_kernel
        )
        vertical_mask = cv2.morphologyEx(
            vertical_source, cv2.MORPH_OPEN, vertical_kernel
        )
        ...
        min_v_length = max(
            14, min(int(binary.shape[0] * 0.08), int(vertical_size * 3.0))
        )
        ...
        if int(np.count_nonzero(vertical_mask)) == 0:
            ...
            fallback_kernel = cv2.getStructuringElement(
                cv2.MORPH_RECT, (1, fallback_v_size)
            )
        ...
        need_vertical_recovery = vertical_count == 0 or vertical_count < max(
            10, int(0.02 * max(1, horizontal_count))
        )
        if need_vertical_recovery:
            hough_v = self._hough_vertical_mask(
                binary, min_line_length=max(10, int(min_v_length * 0.7))
            )
...
def detect_lines(
    binary: np.ndarray, min_line_scale: float = 1.7
) -> LineDetectionResult:
