...
class IntersectionDetectionStage(PipelineStage):
    ...
    def _extract_masks(self, doc: DocumentImage) -> tuple[np.ndarray, np.ndarray]:
        ...
        if horizontal is None or vertical is None:
            raise ValueError(
                "IntersectionDetectionStage requires horizontal_mask and vertical_mask in metadata"
            )
    ...
    def process(self, doc: DocumentImage) -> DocumentImage:
        ...
        min_area, max_area, max_aspect_ratio = _intersection_thresholds(
...
def detect_intersections(
    horizontal_mask: np.ndarray, vertical_mask: np.ndarray
) -> IntersectionResult:
    ...
    return IntersectionResult(
        intersection_mask=intersection_mask, centroids=centroid_list
    )
