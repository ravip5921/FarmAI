...
class TestLineDetectionHelpers(unittest.TestCase):
	def test_kernel_size_uses_average_character_height(self) -> None:
		...
		with patch("src.table.line_detection.estimate_character_size", return_value=report):
	...
	def test_process_recovers_sparse_verticals_from_hough(self) -> None:
		...
		with patch("src.table.line_detection.estimate_character_size", return_value=report), patch.object(
			LineDetectionStage,
